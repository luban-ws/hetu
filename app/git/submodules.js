...
ipcMain.on(
  "Repo-GetSubmoduleDetails",
  requireArgParams(getSubmoduleDetails, ["name"])
);
...
function getSubmoduleNames() {
  if (Repo && window) {
    Repo.getSubmoduleNames().then((names) => {
      ...
      window.webContents.send("Repo-SubmoduleNamesRetrieved", {
    ...
    NodeGit.Submodule.lookup(Repo, arg.name).then((submodule) => {
      ...
      submodule
        ...
        .then((cmt) => {
          ...
          event.sender.send("Repo-SubmoduleDetailsRetrieved", {
            result: result,
          });
        });
    });
