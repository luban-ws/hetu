...
function forkJoin() {
  ...
  if (typeof sources[sources.length - 1] === "function") {
    ...
    return forkJoinInternal(sources, null).pipe(map$1(function(args) {
      ...
      subscriber.add(source.subscribe({
        ...
        complete: function() {
          ...
          if (completed === len || !hasValue) {
            if (emitted === len) {
              subscriber.next(keys2 ? keys2.reduce(function(result, key, i3) {
...
function fromEvent(target, eventName, options, resultSelector) {
  ...
  if (resultSelector) {
    return fromEvent(target, eventName, options).pipe(map$1(function(args) {
...
function transition_selectAll(select2) {
  ...
  for (var groups = this._groups, m = groups.length, subgroups = [], parents = [], j = 0; j < m; ++j) {
    for (var group = groups[j], n = group.length, node, i = 0; i < n; ++i) {
      if (node = group[i]) {
        for (var children2 = select2.call(node, node.__data__, i, group), child, inherit2 = get$3(node, id2), k = 0, l = children2.length; k < l; ++k) {
