...
function consolidateAuthError(err) {
  if (
    err.message.indexOf("credentials") !== -1 ||
...
function pullFFOnly(remoteBranch, currentBranch) {
  return NodeGit.Graph.aheadBehind(
    Repo,
  ...
  return checkSSHKey()
    ...
    .then((result) => {
      if (stashed) {
        ...
        return NodeGit.Stash.pop(
          Repo,
          0,
          NodeGit.Stash.APPLY_FLAGS.APPLY_DEFAULT
        ).then(() => {
          return result;
        });
      } else {
...
function push(
  ...
  if (force) {
    ...
    ref = `+${currentBranch.name()}:refs/heads/${pushedRemote
      .shorthand()
      .replace(`${firstRemote.name()}/`, "")}`;
  } else {
    ref = `${currentBranch.name()}:refs/heads/${pushedRemote
      .shorthand()
      .replace(`${firstRemote.name()}/`, "")}`;
  ...
  return tryPush(firstRemote, [ref], 1, username, password)
    ...
    .then(() => {
      ...
      if (!remoteBranch) {
        return NodeGit.Branch.setUpstream(
          currentBranch,
          firstRemote.name() + "/" + pushedRemote.shorthand()
        ).then(() => {
  ...
  if (Repo && window) {
    ...
    return NodeGit.Stash.foreach(Repo, (index, msg, id) => {
      ...
      return walker
        ...
        .then((result) => {
          window.webContents.send("Repo-CommitsUpdated", {
            newCommits: result,
          });
          return result;
        });
...
function getReferences() {
  if (Repo && window) {
    return Repo.getReferences(NodeGit.Reference.TYPE.LISTALL).then((refs) => {
      ...
      window.webContents.send("Repo-RefRetrieved", {
...
function openRepo(workingDir) {
  let repoName;
  return GitAdapter.Repository.open(workingDir).then((res) => {
    Repo = res;
    let paths = workingDir.split(require("path").sep);
    repoName = paths[paths.length - 1];
    window.webContents.send("Repo-OpenSuccessful", {
      repoName: repoName,
  ...
  if (window) {
    return getCurrentRemotes().then((remotes) => {
      if (remotes.length > 0) {
        ...
        if (
          helper.isSSH(url) &&
          (!settings.get("auth-pubpath") || !settings.get("auth-keypath"))
        ) {
  ...
  if (window) {
    return Repo.getCurrentBranch()
      .then((ref) => {
        ...
        window.webContents.send("Repo-BranchChanged", {
          name: branchName,
          fullName: ref.name(),
          shorthand: ref.shorthand(),
          target: ref.target().toString(),
    ...
    .then((hunkLines) => {
      ...
      requestedLines.forEach((l) => {
        hunkLines.forEach((hl) => {
          if (
            hl.newLineno() === l.newLineno &&
            hl.oldLineno() === l.oldLineno
          ) {
...
function getAllHunkLinesInDiff(diff, path) {
  return diff
    .findSimilar({ renameThreshold: 50 })
    .then(() => {
      return diff.patches();
    })
    .then((patches) => {
  ...
  if (name && email) {
    ...
    return Repo.refreshIndex()
      ...
      .then((ref) => {
        ...
        return Repo.createCommit("HEAD", signature, signature, message, oid, [
          ref.target(),
        ]);
      })
      .then((hash) => {
...
function createBranch(name, commit, force) {
  ...
  return Repo.createBranch(name, commit, force)
    .then((ref) => {
...
function createTag(targetCommit, name) {
  return Repo.getCommit(targetCommit)
    .then((cmt) => {
...
function deleteBranch(name, username, password) {
  ...
  return Repo.getCurrentBranch()
    ...
    .then(() => {
      return findMatchingRemote(branch)
        .then((ref) => {
          if (ref) {
