...
import readLastLine from "read-last-line";
...
function trimCachedFiles(sizeLimit) {
  getSize(cacheDir, (err, size) => {
    ...
    if (size > sizeLimit) {
      window.webContents.send(IPC_EVENTS.CACHE.AUTO_CLEAN_BEGIN);
      ...
      fs.readdir(cacheDir, (err, files) => {
        ...
        stats.sort(function (a, b) {
          return a.mtime.getTime() - a.mtime.getTime();
        });
        let freedSize = 0;
        let toDelete = [];
        ...
        window.webContents.send(IPC_EVENTS.CACHE.AUTO_CLEAN_SUCCESS);
      });
    }
  });
}
...
export { init, getCache, writeCache, downloadFile };
